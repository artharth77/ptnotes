...
- The **web UI** (`ptnotes web`, `src/main/rpc/httpServer.ts`) exposes the same RPC registry over HTTP from the main process: it must keep token/session auth (constant-time compare, `HttpOnly; SameSite=Strict` cookie or `Authorization: Bearer`), the loopback `Host` allowlist, same-host `Origin` checks on POST, the body-size cap, and `GET /api/file` realpath-restricted to the project root (never replicate the desktop `ptfile://` protocol's absolute-path freedom). It may only expose channels already registered on the `rpc` registry — never a broader filesystem/network surface.
- `window.ptnotes` has exactly one definition: `createApi(transport)` in `src/shared/api.ts`. The preload binds it to IPC, `src/renderer/src/webBridge.ts` binds it to HTTP+SSE. Add/rename channels there **and** register them with `rpc.handle`/`rpc.on` in `src/main/ipc/**` — never fork an API shape per transport.
...
- Main-process handlers take `InvokeCtx` (`{ send, platform }`) — never an `IpcMainInvokeEvent`. Native dialogs, reveal and "open with system app" go through `ctx.platform.*` / `getPlatform()`, never a direct `dialog`/`shell` import in `src/main/ipc/**`; browser-unavailable branches use `ctx.platform.mode === 'web'` and answer with `putBlob` + `emitClientAction`. Broadcasts use `broadcast(channel, payload)` (`src/main/rpc/bus.ts`), never a `BrowserWindow.getAllWindows()` loop.
- Planner schedule ids are validated with `validateScheduleId` (same guard as note ids) before building file paths.
