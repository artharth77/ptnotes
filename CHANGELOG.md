...
- **Built-in MCP server** — Settings ▸ **MCP Server** can now expose PTNotes to external MCP clients (Claude Desktop, Cursor, …) over a **loopback-only Streamable HTTP** endpoint by default (`http://127.0.0.1:<port>/mcp`, default port 3737, bearer-token auth). Disabled by default; the pane shows live status, tools/sessions, the token (copy/regenerate), a configurable port, **per-category toggles (Notes / Kanban / Planner)**, an explicit **Allow network access** option that binds to `0.0.0.0` and accepts any `Host` header, detected non-internal IPv4 endpoint selection, and a copy-ready client config snippet. Network mode retains bearer-token auth and displays a warning that plaintext HTTP is trusted-network-only. The server exposes `list_projects` plus only the enabled categories' tools (up to 19), and **every project tool requires an explicit `project`**; file, skill, web, module and browser tools are not exposed. Destructive deletes require MCP client elicitation support (clients without it get the cancelled result). Config (enabled/port/token/categories/listenOnAllInterfaces) is stored in `userData/ptnotes-settings.json` (chmod 600); new IPC: `mcpServer:getStatus` / `mcpServer:update` / `mcpServer:regenerateToken`.
...
- **Scheduled jobs (v.01)** — a new **alarm** button in the vertical tab bar (above the Calendar button) opens a **Schedule Jobs** overlay: the job list on the left and the detail editor on the right, with a toolbar offering New job, Delete (with a confirmation dialog), Enable/Disable, and **Execute now**. Each job defines a title, enable switch, day-of-week set (all selected by default), a time rule (every hour from HH to HH at MM; every 30 minutes at 00/30; every 10 minutes at 00–50; or a specific `HH:MM` list), an execution condition (**meet exactly time** with a +2-minute window after the set time and a 3-minute dedupe — the default — or a pre-computed **next execute time** that fires once reached), a prompt, and a preferred response language (blank = English). Jobs persist per project in embedded SQLite (`<project>/.data/jobs/jobs.db` — nothing to install).
- **Scheduled jobs: minute-tick background runner** — a main-process tick aligned to the :00s second checks every project's enabled jobs and fires due ones (single-flight per job — an `Execute now` burst can never double-start a run). Every run is recorded in the job's history (last 5 shown in the detail panel), executes through the default AI provider profile with **module tools** (`start_module`/`wait_modules`) available, and gets a project-context system prompt (project name, current date/time, preferred response language; the AI can keep working silently by answering exactly `**NO RESPONSE**`). Run history + raw AI traces live under `<project>/.data/jobs/` — separate from chat, groups and modules — and both are purged when older than 30 days.
- **Scheduled jobs: notifications** — a job result (unless the AI answered `**NO RESPONSE**`) appears as a frosted **notification block stacked bottom-right** (job title, time, markdown-rendered body, ✕ close); simultaneous jobs each get their own block, never replacing each other. When the app window is not focused, the Windows taskbar icon flashes / the macOS dock icon bounces, and the flash clears on focus.
- **Scheduled jobs: run history popup with AI traces** — the Jobs overlay toolbar gains a history-clock button on the right that opens a popup listing the selected job's last 20 runs (status badge, start time + duration, notice/error snippet), live-refreshing as runs finish; each run has a raw-AI-trace button that opens the shared trace viewer even while the run is still executing.
- **Settings: vertical tab bar style options** — the Appearance pane gains **vtab style** (icons / labels) and **icon color** (mono / color) choices, persisted with the other appearance settings.
...
- **Diagrams: mermaid edge/node labels are now rendered at the correct position** — under the headless svgdom+resvg renderer, edge labels like `A[Christmas] -->|Get money| B(Go shopping)` were drawn offset from (and often partly outside) their gray background box, and node labels sat slightly off-center in their shapes. Cause: mermaid's SVG-text label markup relies on the browser DOM (`getBBox()` over em-based tspan rows, font size only via CSS, per-tspan `text-anchor` chunks), and svgdom's measurement for that markup disagrees with resvg's glyph layout, and the two engines even resolved different fallback fonts. Fix (all in the isolated renderer, `src/main/modules/shared/mermaid.ts`): every engine is pinned to the Open Sans TTF that svgdom bundles (`fontFamily` config for svgdom, the same file fed to `@resvg/resvg-js` so `svgdom/fonts/**` is now `asarUnpack`ed); after `mermaid.render` a normalizer rewrites each edge and node label deterministically — one `<text text-anchor="middle">` per row at explicitly computed baselines (no em/dy chunking), the edge-label background rect sized from a stable start-anchored measurement (with 8% width slack for font substitution in the `<img>` preview) and centered on the edge midpoint, and node rows centered on the node's label-container rect center. Added a pixel-level regression test in `scripts/test-modules.mts` asserting the label ink renders inside the gray box.
- **Diagrams: mermaid edges now attach to the drawn shape borders** — for diamond/decision shapes (`Y{...}`), the layout pass recorded node dimensions that disagreed with the drawn polygon, so the raw dagre edge points started/stopped dozens of px outside the shape border (visible gaps like `Decide -->|Yes| Pay` not touching the diamond). Fix: after render, the normalizer decodes each edge's `data-points` and snaps its first/last point onto the source/target node's actual drawn boundary (rect / polygon / ellipse ray intersection) when it overshoots, rewriting both `data-points` and the path `d`. Regression test in `scripts/test-modules.mts` asserts a diamond edge anchor sits on the diamond's half-height. Edge-label boxes are also forced to a solid background (`opacity: 1; fill: #e8e8e8` inline style) so they fully mask the edge line beneath them instead of mermaid's default 0.5-opacity translucent look. ER entity nodes are left untouched by the label normalizer so their table name stays in the entity's own header band instead of being re-centered into the middle of the table block (regression test added). The Open Sans TTF that both engines share is also embedded into every rendered SVG as a data-URI `@font-face`, so the editor preview (`<img>`, which cannot load external fonts) draws with the exact glyphs the geometry was laid out for on machines where the font is not installed.
- **Diagrams: wrapped labels now size their containers correctly (kanban & flowchart)** — svgdom's `getBBox()` treated a multi-row label (`tspan y="…em" dy="1.1em"` rows) as a single line, so flowing titles rendered one line shorter-tall than their ink: kanban ticket boxes overlapped their second/third rows and column bounds trailed off. The renderer now installs a text-`getBBox()` polyfill into the svgdom shim that integrates mermaid's row markup the way a browser does (per-row width via `getComputedTextLength`, baselines from the row `y`/`dy` em attributes, Open Sans ink extents — the same font file resvg rasterizes with), so every downstream measurement (item heights, section rects, dagre layout) matches the painted output. Regression test in `scripts/test-modules.mts` asserts a 2-line kanban item is taller than a 1-line item.
- **Editor: mermaid viewer floating buttons fade in/out with the cursor** — the zoom/reset and lightbulb buttons in the split/preview diagram pane are now hidden (0.18s fade) whenever the pointer is outside the viewer and appear on hover (`:focus-within` keeps them reachable via keyboard).
- **Editor: full-screen diagram preview** — the mermaid viewer's floating buttons gain a fullscreen icon (Web Fullscreen API on the viewer canvas): the pane expands to the whole screen with the diagram refit to the new size, the button switches to an exit icon while active, and Esc / clicking it again returns to the split/preview layout.
- **Editor: enlarged diagram preview reuses the shared lightbox** — the viewer's fullscreen button now opens the same `ImageViewer` overlay used by the file explorer and markdown image previews instead of the Web Fullscreen API: the rendered diagram is upscaled to fill the viewport (small diagrams grow, `ImageViewer` gained a `large` mode), with Esc / outside click / ✕ closing it and none of the mermaid-specific fullscreen CSS left over. (The lightbox mounts as a sibling of the viewer canvas so its ✕/outside-click is not swallowed by the canvas pointer capture.)
- **Editor: light/dark canvas toggle in the diagram lightbox** — the mermaid lightbox shows a round floating button (same 36px round size as the ✕, sitting left of it) that switches the overlay canvas between theme-colored dark and white-mode; visible only on dark themes, and the pictorial light copy keeps ✕/caption readable on the white scrim. The file explorer and markdown image lightboxes mount the toggle too, so the light/dark canvas choice is shared across all `ImageViewer` users.
- **Diagrams: event-modeling block labels are now visible** — `eventmodeling` renders its block text as `<foreignObject>` HTML, which resvg (and the `<img>` preview) cannot draw, so every block appeared label-less. The normalizer now inlines each `g.em-box` label as a plain SVG `<text>` (word-wrapped, bold, centered on the box rect) and drops the foreignObject; regression test in `scripts/test-modules.mts`.
- **Diagrams: ishikawa fish-head title now sits inside the half-circle** — the stylesheet forces `text-anchor: middle` on the head label while svgdom measured the rows anchored start, pushing the title across the head's flat left edge. The normalizer re-anchors the label to middle with its vertical placement kept and centers the block at the head arc's middle; regression test in `scripts/test-modules.mts`.
- **Editor: mermaid previews no longer keep the stale diagram on error/empty source** — when a diagram fails to render (or the source becomes empty / the block leaves split/preview mode), the previously rendered image is dropped along with its "last successful render" note; split/preview now shows just the error message (or nothing), and the old image is also cleared as soon as a new debounced render kicks off so it never lingers behind "Rendering…".

### Added

- **Editor: images render in the note editor + per-project image gallery** — image markdown (`![alt](src)`) now displays inside the editor via a TipTap Image node (round-trips through save/reload and raw mode, covered in `scripts/test-editor.mts`). Hovering an image fades in a floating zoom button (same 0.18s pattern as the mermaid viewer) that opens the shared `ImageViewer` lightbox. A new **Insert image** toolbar button opens an image gallery modal backed by a per-project gallery folder at `<project>/notes/images/`: thumbnails render through the existing `ptfile://` protocol, **Add images** picks files via a multi-select OS dialog, image files can be dragged & dropped into the modal, thumbnails are multi-selectable and **Insert** places them all at the cursor as markdown. Stored filenames are `<sha256-16>-<slug(original)>.<ext>` — importing the same file again reuses the existing copy instead of duplicating it, and a per-thumbnail delete (with confirm) removes gallery files. Dragging image files straight into the editor (or pasting them, including clipboard screenshots with no file path — a byte-transfer `gallery:importData` IPC handles those) imports them into the gallery and inserts them at the drop position / cursor, with a dashed drop overlay on the editor. Markdown stores gallery images as relative `images/<name>` paths (portable across project moves); both the editor and the read-only markdown views resolve them against the active project's `notes/` dir over `ptfile://`, so chat, kanban comments and the file viewer render them too. New `gallery:list / import / importData / choose / delete` IPC surface.
- **Editor: mermaid diagram code blocks with Edit / Split / Preview** — ```mermaid fenced blocks now render live: each block gets a header with a three-way mode switch — **Edit** (plain code, the default), **Split** (code left, rendered diagram right in a fixed 50/50 grid; stacks vertically when the editor is narrow) and **Preview** (diagram replaces the code pane, source kept in the document). While split/preview is on, edits re-render automatically, debounced ~500 ms; parse errors show inline and the last good diagram is kept. The mode is a per-block, session-only attribute updated with `addToHistory: false` (never serialized into the markdown and outside undo history); saved notes still contain the plain mermaid fence. Rendering goes through the new `diagrams:render` IPC, which uses the same isolated mermaid utility-process renderer as the module tools (SVG only, no PNG), and the SVG is embedded as a base64 data-URI `<img>` (XSS-safe, CSP already allows `img-src data:`). `mermaid` was added to the code-block language list (no-op highlight grammar so the label round-trips through save/reload) and mermaid markers (`flowchart`, `sequenceDiagram`, `stateDiagram`, `classDiagram`, `erDiagram`, `pie`, `gantt`, …) feed the language auto-suggest. Split/preview refinements: the code pane is fixed at ~20 rows with no-wrap text and two-axis scrolling (bg-colored gutters keep a 12px inset when scrolled), and the diagram pane is an interactive viewer — fit-to-pane on load, drag to pan, floating buttons for zoom in/out and reset, plus a lightbulb toggle group (dark themes) that switches the canvas to white-mode colors; viewer icons are 16px. **Cmd/Ctrl+A inside a code block selects only that block's text** — the Edit menu's Select All now routes `global:select-all` IPC to the note editor, which expands the block selection or falls back to the native behavior. Covered in `scripts/test-editor.mts`.
...
- **Planner: file snapshots** — every schedule save now keeps an automatic on-disk history in `<project>/.data/snapshots/planner/<schedule>/` and surfaces it in a **Snapshots** dialog (history icon on each schedule row or the schedule context menu). A snapshot is captured only when the schedule's content actually changed (sha256 of the schedule minus its `updatedAt` stamp, so no-op saves are skipped) and the retention prunes automatically, grandfather-father-son style: one snapshot per minute for the last 10 minutes, one per hour for the last 24 hours, one per day for the last week, one per week for the last month — anything older than 30 days is purged. **Tagging a snapshot (a short user label, e.g. "before restructure") pins it — tagged snapshots are never purged** and stay until explicitly deleted (delete asks for confirmation). The dialog lists snapshots newest-first with time, tag badge, hash and size, previews the snapshot's top-level tasks on click, and can **Restore** any snapshot — restoring first snapshots the current state, so a restore is always reversible. Snapshots move with a schedule when it is renamed and are removed when the schedule is deleted.
- **Planner: Export to Excel** — a new toolbar button (after the Calendar button) opens an **export options modal** before saving: a **planner date** picker (default today), a **progress line** choice (Progress only, or Progress + plan — the plan percent is the `estimatePercentComplete` projection for the planner date), and a **Gantt chart** choice (None / Day / Week). Exporting then writes the `.xlsx` via a native save dialog (default `<schedule name>.xlsx` in Downloads). The sheet is named after the schedule (invalid characters replaced, 31-char cap, "Schedule" fallback) and carries a title block in column C — the schedule name (C2, Calibri 14 bold), `Date: <min plan start> - <max plan end>` (C3), `Progress date: <planner date>` (C4) and the progress line (C5, Calibri 12): `Progress: <overall %complete>%` or `Progress: <overall %>% (plan <projected %>%)` when the plan percent is requested — above the table: the visible columns in their current order (header row 8, uppercased, white bold on Dark Blue), the full task tree indented by depth, bold titles for tasks with children, root rows tinted Blue Accent 5 Lighter 60%, status cells tinted Green Accent 6 Lighter 40% (Completed) / Gold Accent 4 Lighter 80% (In Progress), dates as `dd-MMM-yyyy`, %Complete as a number with a `%` sign — for rows with children it is a live Excel formula computing the duration-weighted mean of their direct children (`SUMPRODUCT` over the child rows, plain `AVERAGE` fallback when no child has a duration, or when the Duration column is not exported), so parents recalculate when children are edited in Excel, No. right-aligned and Status/Owner/Duration/date columns center-aligned, 2.33-wide margin columns flanking the table, Calibri 11 throughout, thin white borders around the whole block and thin White-Background-1-Darker-15% borders around the table. When the Gantt is enabled it is embedded two columns right of the table (one 2.33-wide margin column between): **Day** mode has one 2.6-wide column per day spanning min Plan Start − 7 to max Plan End + 7 days (planner date ± 7 when no task has dates) — row 5 carries the left-aligned month label merged across each contiguous month (with the year when the range spans multiple years), row 6 a compact single-letter weekday, row 7 the day-of-month (non-working days shaded light gray per the project calendar, the planner-date column tinted light red), row 8 a merged "PLAN" banner on the header fill — and each task row paints a bar over its Plan Start..Plan End days (parent rows use their rolled-up dates) — Blue Accent 5 (#5B9BD5) for tasks with children, lighter Blue Accent 5 Lighter 60% (#BDD7EE) for leaves, with thin table borders across the whole grid. **Week** mode has one column per week at the same width as a day column (weeks run from the calendar's week start, the range snapped to week boundaries): row 5 month labels merged per contiguous month (the month of the week's first day), row 6 `W1`/`W2`/… where the number resets to `W1` at each new month, row 7 the first day-of-month of the week (the week containing the planner date tinted light red), the same "PLAN" banner, and a week cell is filled when the task's plan range overlaps the week (no non-working-day shading).
- **Planner: Estimate %Completed** — a new toolbar button (before the View columns button, separated by a divider) opens a read-only modal with the current root %Complete, an estimate date picker (default today), and the projected %Complete for that date: leaf tasks planned to finish on or before the date count as 100%, other leaves keep their entered value, and parents are duration-weighted averages (`estimatePercentComplete`).
- **Planner: column reordering** — the View columns dialog now reorders the movable columns with up/down arrow buttons; Plan Indicator, No., and Title stay pinned at the front and cannot be moved. The order is stored per schedule (`columnOrder`) and normalized on load (`normalizeColumnOrder`), so a saved order missing keys or containing unknown ones is repaired instead of breaking the grid. A Reset button in the dialog restores the default column visibility and order.
- **Planner: Plan Indicator column** — a new 5px color strip rendered before the No. column (sticky with it), computed per row from the record and today's date: green when %Complete = 100, yellow when < 100 and today falls between Plan Start and Plan End (inclusive), red when < 100 and today is past Plan End, not filled when < 100 and today is before Plan Start (or the task has no plan dates). The value is derived at render time and never stored; the column is shown by default and can be hidden in the View columns dialog.
- **Planner: duplicate schedule** — the schedule context menu (right-click a row or its ⋮ button) gains **Duplicate**: clones the selected planner file as "name (copy)" (auto-suffixing "(copy 2)", "(copy 3)" … on conflicts) with fresh created/updated timestamps, all tasks and rollup fields intact, and opens the copy in the editor.
- **Files: project file explorer** — new icon-only **Files** tab (folder icon) in the left sidebar: the sidebar shows a folder tree of `<project>/files` and the working panel shows the file list (Name / Size / Modified, folders first, per-type icons, rows follow the cozy/compact density setting) with clickable breadcrumbs in a bottom statusbar. Toolbar: New Folder / Copy / Cut / Paste / Rename / Show in Folder / Delete (confirm dialog). Multi-select via click / ctrl-click / shift-click; double-click drills into folders and a `..` row (double-click) drills out. All paths are traversal-guarded, name conflicts auto-suffix `name (2)`, folder copy/move is recursive and move-into-itself is rejected.
- **Files: context menu + keyboard** — right-click on a row or empty space opens New Folder / Copy / Cut / Paste / Rename / Show in Folder / Delete with the same enable rules as the toolbar (measured and clamped to stay inside the window; right-click selects an unselected row first). Keyboard: `↑`/`↓` move the selection (reaching `..` inside subfolders, scrolling below the sticky header), `Enter` acts like double-click, `⌘/Ctrl+C/X/V` copy/cut/paste, `Delete`/`Backspace` opens the delete confirm.
- **Files: file viewers** — double-click opens images (`png/jpg/jpeg/gif/svg/webp/bmp/ico`) in a lightbox (shared `ImageViewer`, also used by chat images) and markdown (rendered) or plain-text/code files (monospace) in a read-only, scrollable `FileViewer` with a 2 MB preview cap.
- **Files: PDF preview** — double-click (or `Enter`) on a `.pdf` in the file list opens a fullscreen preview rendered by Chromium's built-in PDF viewer: the PDF plugin is enabled on the main window and the file is served over the `ptfile://` protocol into an iframe (dark overlay, file name in the header, ✕/Escape to close). Full viewer toolbar (zoom, page navigation, print, save) comes for free; no extra dependency, no network access.
- **Files: type column + sortable headers** — the file list gains a **Type** column (Folder / PDF / Image / Markdown / Word / Excel / Powerpoint / Text, otherwise the uppercase extension) and clickable column headers: clicking a header cycles that column through ascending → descending → default (folders first, name order), with a direction indicator on the active column. Folders always stay grouped first — the sort applies within folders and within files separately — and drives the visible list, arrow-key navigation and shift-click range selection alike, so the keyboard follows the visual order. Sort choice is session-only (resets on restart).
- **Files: drag & drop import** — drop files or whole folders from the OS anywhere on the list to import them into the current folder (any file type, auto-suffix on conflicts, recursive folder copy, drop overlay); the explorer and the `#` mention picker refresh after import.
- **Chat: `#` file mentions support folders** — the `#` picker now lists project files **and** folders (folders get a folder icon, `mdi/folder-open-outline`). While the popup is open, typing `/` with a folder highlighted (or selecting a folder) drills into it and lists its contents; selecting a file inside a folder inserts `file:<relative/path>` (rows show the entry name, the full relative path as tooltip). Folder/file names containing spaces are supported: drill-in quotes the segment in the input (`#"my folder"/`) and the picker keeps matching across the quoted part. Works in both the 1:1 AI chat and the bots group chat.
- **Files: filter box** — the explorer toolbar gains a right-aligned filter input (reusing the notes filter style): typing narrows the current listing to entries whose name contains the text (case-insensitive substring), an embedded ✕ clears it and so does Escape inside the field. The filter drives the visible list, arrow-key navigation and shift-click range selection alike; while it is active the statusbar shows "X of Y items" and an empty result reads "No matches" instead of "This folder is empty". Like the column sort it is session state and persists across folder navigation.
- **AI: `read_file` reads files inside subfolders** — the tool's `name` (and `file:` chat mentions / `[name](file:…)` links) now accept paths relative to the project files folder, e.g. `file:docs/report.pdf`; absolute paths and `..` traversal are rejected. When a file is not found, the error lists every file in the project recursively (relative paths, capped at 500) so the AI can discover nested files, and the system prompt/bot link rules document subfolder references.
- **AI: `read_file` supports a `page` parameter for long files** — instead of only ever seeing the first 240,000 characters of a long file, the assistant can now read it page by page: `page` is 1-based and means that PDF page for PDFs, and a 240,000-character window for text/Word files (page 1 is exactly what the tool returned before). The result now reports `totalPages` (and the requested `page`), and the system prompt tells the AI to keep calling `read_file` with the next page while there are pages left. Out-of-range pages are rejected with the total, and Excel workbooks reject `page` (use `query=workspace=…` to pick a worksheet).
...
- **Files: PDF page manager** — select a PDF in the Files explorer (toolbar or context menu) and open **Manage Pages**: a fullscreen grid of lazily-rendered page thumbnails (pdf.js + `@napi-rs/canvas`, isolated in an Electron utility process) where pages can be drag-reordered, rotated 90° clockwise per page, and removed/restored. Saving always writes a **new** `name (pages).pdf` beside the original (existing `name (2)` conflict suffix) — the source PDF is never modified. Caps: 50 MB and 500 pages per PDF; encrypted/invalid PDFs are rejected with a clear message.
- **Files: Merge PDFs** — select two or more PDFs in the Files explorer (toolbar or context menu) and open the merge dialog: drag the files into the desired merge order, set the output file name (default `merged.pdf`, `.pdf` extension forced, conflict suffix), and the merged PDF is written into the current folder as a **new** file — sources are untouched. Caps: 20 PDFs, 500 pages and 100 MB total.

### Changed

- **AI: PDF text extraction uses `pdfjs-dist` directly** — the AI's local PDF text extraction no longer goes through the `pdf-parse` wrapper; it now uses the legacy `pdfjs-dist` build directly in the main process while preserving the previous extracted-text formatting.
...
- **Bots: kanban workflow builtin skill (`bot-kanban-workflow`)**: a read-only, app-shipped skill (Settings ▸ Skills ▸ Builtin skills, toggleable) that defines the protocol bots follow when a task involves kanban cards: list the board first; only claim cards sitting in **Backlog or To Do** whose assignee is empty or already your own name (never another bot's/person's), claiming by setting the assignee to your bot display name and moving the card to In Progress; on completion move it to Done and add a result comment; on failure move it back to To Do with an "Attempt N failed:" comment, the `failed` label (merged into the card's existing labels) and a cleared assignee. An unassigned card in any other column (typically In Progress) is never adopted silently — when the task needs it, the bot asks you in the group chat (`ask_user`, with the card's failure history if any) and works it only on an explicit Yes, otherwise skipping and reporting it. Anti-loop rules are part of the protocol: failed cards sitting in Backlog/To Do are frozen (bots never re-pick them on their own), a failed card is retried at most once and only when a task explicitly reassigns it or the user explicitly confirms the takeover, and a recurring same-cause failure is reported instead of retried. Supporting changes: bot-task prompts now open with the bot's identity (`@<id>` + display name) so self-assignment is reliable, and the bot-task system prompt directs kanban work to load the skill first.
- **Bots: kanban comments written by bot tasks carry the bot's name**: the `add_kanban_comment` tool now attributes each comment it writes to the run's bot (`commentBy` = the bot's display name) instead of always writing "you", so a board shows who really posted a result or an "Attempt N failed" note (and the skill's anti-loop rules can tell prior attempts apart). Comments written by the main chat assistant and by you in the card modal are still attributed to "you".
...
- **AI: kanban tool parameters are now documented**: `create_kanban_card` and `update_kanban_card` left `assignee`, `priority`, `labels` and story points without parameter descriptions (bare types only), so models often sent only the card title and silently skipped the fields they meant to set — e.g. a bot claiming a card never wrote the assignee. Every field now carries a description, including the claiming hint ("to claim a card pass your own display name") on `assignee` and the fact that `labels` replaces the card's whole label list. The `bot-kanban-workflow` skill's claim/failure steps now also name the exact tool arguments.
- **UI: group-chat question bubble input ignored the dark theme**: the free-text answer field in a bot task's interactive question bubble was hardcoded white (`background: #fff`) with the theme's text color — in dark mode that made a glaring white box with nearly invisible light text. The field is now a translucent white overlay on the bubble's accent color with white text and placeholder (and a brighter border on focus), matching the bubble's buttons in both themes.
