...
- **AI planner tools** (20th–26th): `list_schedules`, `read_schedule`, `create_schedule`, `update_schedule` (rename), `add_task` (with optional parent nesting; planStart+duration or both dates — the missing value is computed), `update_task` (end-date-fixed date edits, status/percent handling), and `set_calendar` (week + holiday changes that re-roll schedules).
