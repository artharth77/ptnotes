...
- **Planner: Export to Excel** — a new toolbar button (after the Calendar button) exports the current schedule to a `.xlsx` file via a native save dialog (default `<schedule name>.xlsx` in Downloads). The sheet is named after the schedule (invalid characters replaced, 31-char cap, "Schedule" fallback) and carries a title block in column C (Calibri 12) — the schedule name (C2), `Date: <min plan start> - <max plan end>` (C3) and `Progress: <overall %complete>%` (C4, the actual duration-weighted rollup) — above the table: the visible columns in their current order (header row 8, uppercased, white bold on Dark Blue), the full task tree indented by depth, bold titles for tasks with children, root rows tinted Blue Accent 5 Lighter 60%, status cells tinted Green Accent 6 Lighter 40% (Completed) / Gold Accent 4 Lighter 80% (In Progress), dates as `dd-MMM-yyyy`, %Complete as a number with a `%` sign — for rows with children it is a live Excel formula computing the duration-weighted mean of their direct children (`SUMPRODUCT` over the child rows, plain `AVERAGE` fallback when no child has a duration, or when the Duration column is not exported), so parents recalculate when children are edited in Excel, No. right-aligned and Status/Owner/Duration/date columns center-aligned, 2.33-wide margin columns flanking the table, Calibri 11 throughout, thin white borders around the whole block and thin White-Background-1-Darker-15% borders around the table. The export also embeds a **Gantt timeline** two columns right of the table (one 2.33-wide margin column between): one 2.6-wide column per day spanning min Plan Start − 7 to max Plan End + 7 days (today ± 7 when no task has dates) — row 5 carries the left-aligned month label merged across each contiguous month (with the year when the range spans multiple years), row 6 a compact single-letter weekday, row 7 the day-of-month (non-working days shaded light gray per the project calendar, the today column tinted light red), row 8 a merged "PLAN" banner on the header fill — and each task row paints a bar over its Plan Start..Plan End days (parent rows use their rolled-up dates) — Blue Accent 5 (#5B9BD5) for tasks with children, lighter Blue Accent 5 Lighter 60% (#BDD7EE) for leaves, with thin table borders across the whole grid.
