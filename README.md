...
- **MCP Server** — exposes PTNotes projects, notes, kanban cards and planner schedules to external MCP clients (Claude Desktop, Cursor, …) over a loopback-only Streamable HTTP endpoint. Disabled by default; when enabled it shows the URL and a bearer token plus a copy-ready client config snippet, with **per-category toggles** (Notes / Kanban / Planner) controlling which tools are offered. Every tool requires an explicit project name (`list_projects` lists them), so clients never touch the wrong project.
