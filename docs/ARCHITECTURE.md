...
- **MCP server (built-in):** `getStatus` (enabled/running/port/url/token/sessionCount/toolCount/error), `update(patch)` (patch `enabled`/`port`/`token`, persists to settings + (re)starts the loopback server), `regenerateToken` (new bearer token + restart). The server itself is not an IPC surface — external clients speak MCP over `http://127.0.0.1:<port>/mcp`.
...
- **Host** (`src/main/mcp/server.ts`): a `node:http` server bound to **127.0.0.1** only, wrapping the SDK's `StreamableHTTPServerTransport` at `/mcp` in stateful mode (one `Server` per session). Every request is checked for a bearer token (constant-time compare) and a loopback `Host` header. `McpServerHost.reconfigure(settings, service)` starts/stops/restarts on config change; `will-quit` stops it. Port `0` binds an ephemeral port and reports the real one back.
...
- **Config** (`McpServerSettings { enabled, port, token, categories }`) lives in `userData/ptnotes-settings.json` (chmod 600). IPC: `mcpServer:getStatus` / `mcpServer:update` / `mcpServer:regenerateToken`; the renderer pane shows status, port, token, per-category toggles and a copy-ready client config snippet. Changing categories restarts the host (existing sessions drop them).
