...
| [Built-in MCP server](#built-in-mcp-server)                             | External MCP clients, loopback HTTP host, exposed tool set, token, settings pane |
| [Web UI mode](#web-ui-mode)                                             | `ptnotes web` headless server, HTTP/SSE transport, auth, web-only branches       |
...
- `@modelcontextprotocol/sdk` (v1.30) — in-process MCP server + client over `InMemoryTransport` for the browser toolset, `StdioClientTransport` / `StreamableHTTPClientTransport` for user-configured external MCP servers (client side), and a built-in `StreamableHTTPServerTransport` server exposing PTNotes' project tools to external clients over loopback (server side)
...
| Settings dialog     | Two-panel dialog: **Storage** (project root path) + **Appearance** (Light/Dark/System color-scheme modes) + **AI Settings** (profile set: active selector, per-profile base URL/API key/model with endpoint presets, global PDF toggle) + Modules + Toolsets + MCP Server + Skills + Bots + About                                                                                                                                                                                                                                     |
...
| Chat file mention   | `#` opens a project file/folder picker (`files:listEntries` → one level of `<project>/files/<subpath>`; folders shown with a folder icon) → typing `/` while a folder is highlighted (or selecting a folder) drills into it, showing its entries (path segments with spaces are quoted in the input, `#"my folder"/`) → inserts `file:<relative/path>` → AI calls `read_file` (content-based: pdfjs-dist for PDFs, jszip+cheerio for .docx, exceljs for .xlsx/.xlsm, raw text for any text file; `read_file` accepts subfolder paths) |
...
| Files & PDF tools | Per-project `<project>/files` explorer (tree + list, CRUD, drag & drop import, viewers). PDF extras: **Manage Pages** (lazy pdf.js thumbnails, drag reorder, per-page 90° rotate, delete → saves a NEW `name (pages).pdf` via pdf-lib) and **Merge PDFs** (drag-ordered dialog → new merged file, default name `merged.pdf`); originals are never modified |
...
└── <ProjectName>/
    ...
    └── .data/              (app-internal data; dot-prefixed so it stays out of the # file picker)
        ...
        ├── snapshots/planner/<scheduleId>/  (per-schedule file snapshots: `<YYYYMMDDTHHMMSSmmmZ-<hash16>>.json` + `tags.json`)
...
- App settings (project root path + `disabledModules` module toggles + `disabledToolsets` toolset toggles + `browserHeadless` preference + `mcpServer` built-in-server config) stored in Electron `userData/ptnotes-settings.json`, `chmod 600`.
...
│   ├── index.ts         # CLI mode branch + initCore() (shared core) + desktop window/splash/menu or web server
│   ├── platform.ts      # PlatformServices (dialogs/reveal/open) + AsyncLocalStorage client-action sink
│   ├── platformDesktop.ts # desktop impl (dialog parenting to the focused window); webPlatform lives in platform.ts
│   ├── rpc/
│   │   ├── registry.ts  # RpcRegistry singleton + InvokeCtx { send, platform } — one registry for both transports
│   │   ├── ipcAdapter.ts # desktop: binds every channel to ipcMain (one handle/on per channel)
│   │   ├── bus.ts       # broadcast(channel, payload) → all windows (desktop) / all SSE clients (web)
│   │   ├── blobs.ts     # in-memory one-shot download blobs (10-min TTL) for Excel/diagram exports
│   │   └── httpServer.ts # `ptnotes web` node:http server: auth gate, /api/invoke, /api/events, /api/blob, /api/file, static SPA (+ dev proxy)
...
│   │   ├── snapshots.ts # snapshots:list/read/restore/setTag/delete (planner schedule file history)
...
│   │   ├── files.ts     # files:* attach/extract/list/reveal + pdf:upload + pdf:info/renderPage/rebuild/merge (page manager + merge)
...
│       ├── reader.ts     # readFileAsText + detectFileKind: content-based (pdfjs-dist for PDFs, exceljs for .xlsx/.xlsm, raw text for any text file) + MAX_PDF_CHARS truncation
│       └── search/
│           ├── duckduckgo.ts  # web_search (no key)
│           └── webFetch.ts    # cheerio page extraction
│   ├── pdf/
│   │   ├── ops.ts              # pdf-lib page ops (pure buffers): pdfPageCount + rebuildPdfPages (reorder/delete/rotate) + mergePdfs
│   │   ├── pdfRenderCore.ts    # pdf.js legacy + @napi-rs/canvas page renderer (open/render/close doc handles → JPEG thumbnails)
│   │   ├── pdfRenderer.ts      # host: isolates rendering in an Electron utilityProcess (crashes contained; plain-Node fallback) + doc cache (2 docs)
│   │   └── pdf-render-worker.ts # utility-process entry (open/renderPage/close protocol)
│   └── bots/
...
├── preload/             # contextBridge: createApi(ipcTransport) → window.ptnotes (API shape lives in shared/api.ts)
├── renderer/            # React app
│   ├── src/
│   │   ├── App.tsx
│   │   ├── webBridge.ts  # web mode: createApi(httpTransport) over fetch + EventSource (installed only when no preload ran)
│   │   ├── uiMode.ts     # isWebUi() + reveal label/icon switching (Show in Folder → Download)
...
│   └── ...
└── shared/
    ├── api.ts           # createApi(transport) — the ONE definition of window.ptnotes (IPC and HTTP transports)
    ├── cliArgs.ts       # PURE CLI parsing (`web` subcommand + flags + usage) shared by main + tests
    ├── rpcCodec.ts      # Uint8Array ↔ { __b64 } codec for the JSON HTTP transport
...
- The **web UI server** (`ptnotes web`, `src/main/rpc/httpServer.ts`) is a `node:http` server in the **main** process that exposes the very same RPC registry the renderer uses over IPC — it must never widen the surface beyond the existing channels. It requires a session (random token or `--token`) delivered as an `HttpOnly; SameSite=Strict` cookie (or `Authorization: Bearer`), compares tokens with a constant-time compare, and when bound to loopback validates the `Host` header against `127.0.0.1`/`localhost`/`::1` to block DNS rebinding; `--no-auth` is the only way out and is announced as trusted-network-only. `GET /api/file` must stay realpath-restricted to the current project root (the desktop `ptfile://` protocol's absolute-path freedom must not be replicated), request bodies are capped (128 MB), and `Origin` (when present) must match the host. The server binds only from `ptnotes web` — never from the desktop UI.
...
The channel names and shapes below are the contract for **both** transports: the desktop preload maps them 1:1 onto `ipcRenderer`, and the web UI maps them 1:1 onto `POST /api/invoke` (see [Web UI mode](#web-ui-mode)). Add a channel in `src/shared/api.ts` (`createApi`) **and** register it with `rpc.handle` — the two must stay in step.
...
- **Snapshots:** `list(project, scheduleId)` (metas newest-first, tags merged), `read` (one snapshot's `Schedule` or `null`), `restore` (captures the current content first, then writes the snapshot back with the current id + fresh `updatedAt`), `setTag` (`null` clears), `delete` (explicit removal — the only way a tagged snapshot disappears). See [Planner snapshots](#planner-snapshots).
...
- **MCP server (built-in):** `getStatus` (enabled/running/port/url/token/listenOnAllInterfaces/listenAddress/networkUrls/sessionCount/toolCount/error), `update(patch)` (patch `enabled`/`port`/`token`/`listenOnAllInterfaces`, persists to settings + (re)starts the host), `regenerateToken` (new bearer token + restart). The server itself is not an IPC surface — local clients speak MCP over `http://127.0.0.1:<port>/mcp`; after explicitly enabling all-interface access, `networkUrls` contains locally detected non-internal IPv4 endpoints for selection in the client configuration.
...
- **PDF:** `supportsUpload` (returns the AI settings `uploadPdfEnabled` toggle — user-controlled), `upload` (raw PDF via provider Responses API `input_file` — uploads base64 through the Files API, falling back to inline `file_data`; takes `sessionId` so the upload exchange is traced into the session's trace file), `info` (page count + base rotations via the isolated pdf.js render worker; 50 MB / 500-page caps), `renderPage` (one JPEG page thumbnail at ~200px width, optional 0/90/180/270 rotation), `rebuild` (pdf-lib page rebuild — ordered `PdfPageEdit[]` reorders/deletes/rotates pages; writes a NEW `name (pages).pdf` beside the source and returns its relative path), `merge` (pdf-lib merge of the selected PDFs in array order into `destSubpath` as `destName` — default `merged.pdf`, `.pdf` forced, conflict suffix; 20 files / 500 pages / 100 MB caps)
...
- A `#` file mention inserts `file:<filename>` (or `file:<subfolder>/<filename>` picked by
  ...
  `.pdf` via pdfjs-dist, `.docx` via jszip+cheerio as markdown-style text, any text file as raw
...
| `add_task`              | add a task (optional parent nesting); planStart+planEnd or planStart+duration — the missing value is computed; `addAfter` without `parent` infers the parent (sibling of the matched task); batch mode via `tasks` array (up to 30 records per call; top-level fields are defaults, per-record overrides; per-record ok/error results; no code-enforced limit)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `update_task`           | update a task's fields (match by id, task number or title); plan date edits re-derive the other value (end-date-fixed); plan-field edits on parents are rejected (derived from children); `parent`/`addAfter` moves the task (and its subtree) — `addAfter` without `parent` infers the parent (sibling of the matched task), explicit empty `parent` → top level, cycle-safe; batch mode via `tasks` array (up to 30 records per call; top-level fields are defaults, per-record overrides; per-record ok/error results; no code-enforced limit)                                                                                                                                                                                                                                                                                                                                    |
...
- Dropping one or more files onto the chat drawer copies each **supported** file (any text file —
  ...
  messages containing `file:<filename>` are handled by the `read_file` tool (local `pdfjs-dist` for
...
Automatic per-schedule file history (phase 1 covers planner schedules; notes may reuse the same
helpers later).

- **Storage**: `<project>/.data/snapshots/planner/<scheduleId>/<YYYYMMDDTHHMMSSmmmZ>-<hash16>.json`
  (UTC stamp, Windows-safe, sorts lexicographically = chronologically; `hash16` = first 16 hex of
  the sha256 of the **canonical** schedule JSON — the schedule minus `updatedAt`, which is stamped
  on every save, so a no-op save never hashes as new) + `tags.json` (map of filename →
  `{ tag, taggedAt }`; corrupt/missing tolerated as empty). Snapshot dirs are keyed by slug and
  **move with the schedule on rename** (tags included); schedule delete removes the dir.
...
- **Retention** (GFS cascade, `planSnapshotPrune` in `src/shared/snapshots.ts`, newest-per-bucket):
  ...
  earlier state A → B → A never keeps two snapshots with the same hash — the newest one mirrors the
  live content). **Tagged snapshots are exempt** (the `tags.json` entry is the source of truth)
  until explicitly deleted via `snapshots:delete`.
- **Restore** (`restoreSnapshot`): writes the restored content through the locked save path, then
  captures the overwritten content forced + nudged one minute back — in that order each side lands
  in its own retention bucket and no same-hash duplicate pair exists at prune time, so a restore is
  always reversible. The snapshot is written back with the current schedule id and a fresh
  `updatedAt`.
- **UI**: history icon per schedule row (and a schedule context-menu entry) opens
  `PlannerSnapshotsModal` — chronological list (timestamp, tag badge, hash, size), click to preview
  top-level tasks, inline tag edit, restore, and delete-with-confirm.
...
Scheduled background AI jobs: the user defines a job (title, day-of-week set, time rule, execution condition, prompt, preferred response language, **scope**) and a minute-tick scheduler in the main process fires tool-capable background AI runs when due. Jobs are either **project-scoped** (run in one project) or **global** (run the same prompt in every project, then summarize all results into one notification — see [Global scope](#global-scope)).

### Data & storage

- **Per project**: `<project>/.data/jobs/jobs.db` (SQLite via `node:sqlite`, WAL) with tables `jobs` (id UUID, title, enabled, days JSON — JS `getDay()` values defaulting to all seven, time_rule JSON, condition `exact`/`next`, prompt, language, `scope` `project`/`global` defaulting to `project`, next_run_at, last_run_at, timestamps) and `job_runs` (run_id, job_id, title snapshot, started_at/finished_at, status `running`/`done`/`failed`/`cancelled`, notice, error; indexed by started_at). Per-run AI traces append to `<project>/.data/jobs/traces/<runId>.trace.jsonl` (same JSONL shape as chat/bot traces, recorded as kind `module`). Runs left `running` by a crash are reconciled to `cancelled` on the next `jobs:list`. Deleting a job removes its run rows **and** its trace files (`deleteJobWithTraces`).
- **Global**: global-scope jobs live in the root-level DB `<root>/.data/jobs/jobs.db`, addressed by the sentinel key `GLOBAL_PROJECT_KEY = ''` (`JobsStore.jobsDir('')` resolves there). `saveJob` enforces scope/key consistency (a `global` job must be saved under `''`, a `project` job under a non-empty key); updating without a `scope` keeps the stored scope. Changing a job's scope is a **recreate, not an update**: `moveJobScope(src, dest, id, input)` creates a new job (fresh id, fresh run history) in the destination DB, then deletes the old job with its run rows and trace files — create-first, so a failed delete can never lose the job.
...
- The tick always runs an extra pass for the sentinel key `''` after the per-project passes (a `listProjects` failure no longer aborts the tick — it degrades to "no projects" so global jobs still fire). `runnerFor('')` returns a `GlobalJobRunner` (`src/main/jobs/globalRunner.ts`) instead of a `ScheduleJobRunner`.
- `GlobalJobRunner` fans the job prompt out to **every project** by reusing `ScheduleJobRunner` per project with a worker pool capped at **3 concurrent** runs. Each per-project execution is recorded as its own `job_runs` row (title `<job title> · <project>`) **and** its own trace file, both in the root-level jobs DB (children pass `traceProject: GLOBAL_PROJECT_KEY` to the runner).
- After the fan-out, one non-streamed summarizer call (same default profile, honors the job's language) merges all per-project answers into a single notification; per-project sections are capped at 4000 chars and failures are included as `(run failed: …)` sections. With a single project the child answer is passed through verbatim (no summarizer call). If **all** projects fail, the parent run is `failed` (no summary, no notification); a partial failure still summarizes. The parent run's trace records the orchestration: system prompt, job prompt, one `run-project` tool entry per child, then the summarizer exchange.
- The `**NO RESPONSE**` rule applies to the final summary, not the individual child answers.

### Notifications & UI

- `notify` events broadcast to all windows with `webContents.send('jobs:event', …)`; a window that isn't focused also gets `flashFrame(true)` (Windows taskbar flash / macOS dock bounce) and `focus` clears it. Notifications **stack** bottom-right (`JobNotifications.tsx`): one frosted block per run with job title, project label (project name, or "All projects" for global runs via `projectLabel`), time, full text, and its own ✕ close — live events never replace each other, older ones remain until manually closed.
- The vtab strip gains an **alarm** button above the Calendar button (always enabled — global jobs are manageable without an open project); it opens `ScheduleJobsOverlay`: toolbar (New job / Delete with confirm / Enable-Disable / Execute now) on top of a two-pane body — job list on the left, detail form on the right; the detail edits the selected job or, with nothing selected, shows a **new job** form. The list is grouped into **This project** (only with an active project) and **Global (all projects)** sections; every mutation (save/enable/delete/run-now/runs/trace) routes to the job's scope key (`''` for global). Detail fields: title, **Run in** scope selector (This project / All projects — project scope disabled without an active project, new jobs default to the active scope; changing it while editing an existing job prompts a confirmation on save — the job is recreated in the new scope and the old one deleted with its run history and AI traces, cancel returns to the editor with the draft kept), enabled switch, day-of-week checkboxes (all default), time rule select (4 kinds with from/to-hour and minute/list inputs), execution-condition radio (exact ±2 min default vs next-run), prompt textarea, preferred response language (blank = English placeholder). In edit mode, Last executed / Next execute show as read-only info (last from `lastRunAt`, next only meaningful for `next`-condition jobs).
...
PTNotes can expose its project data to external MCP clients (Claude Desktop, Cursor, …). This is the server-side counterpart of the Toolsets pane; it is **disabled by default** and managed in Settings ▸ MCP Server.

- **Host** (`src/main/mcp/server.ts`): a `node:http` server wrapping the SDK's `StreamableHTTPServerTransport` at `/mcp` in stateful mode (one `Server` per session). It binds to **127.0.0.1** by default and validates a loopback `Host` header to block DNS rebinding. The user may explicitly opt into `listenOnAllInterfaces`, which binds to **0.0.0.0** and accepts arbitrary `Host` headers for clients on a trusted network. The main process enumerates non-internal IPv4 addresses with `node:os.networkInterfaces()` for display; it never performs a public-IP lookup. Bearer-token authentication is mandatory in both modes and uses a constant-time compare; an enabled server with no token fails closed. `McpServerHost.reconfigure(settings, service)` starts/stops/restarts on config change; `will-quit` stops it. Port `0` binds an ephemeral port and reports the real one back. The all-interface mode is plaintext HTTP, so the Settings UI displays a trusted-network-only warning and requires confirmation.
- **Tool surface** (`src/main/mcp/ptnotesServer.ts`): a fixed, deliberate set built from the chat's `PTTool`s — `list_projects` plus notes (5), kanban (6) and planner (7) tools; up to 19 total. File/skill/web/module/browser/`ask_user` tools are **not** exposed. Uses the low-level SDK `Server` (raw JSON Schema from the shared tool definitions) with `ListToolsRequestSchema` / `CallToolRequestSchema`. The notes/kanban/planner categories are individually toggleable (`McpServerCategories`); `buildMcpTools(categories)` filters the list (and each session is built with the categories active at start), while `list_projects` is always present.
- **Project scoping:** every tool except `list_projects` gets a **required** `project` schema property injected (the shared definitions are copied, never mutated) and the call is rejected if it is missing.
- **Destructive tools:** `delete_note` / `delete_kanban_card` map `ctx.confirm` to an MCP **elicitation** (`elicitation/create`); a client that does not advertise the `elicitation` capability cannot delete — the tools return their standard cancelled result. Secrets in kanban attributes use a per-session in-memory map (`registerSecret` + `resolveSecretTokens`), never persisted.
- **Config** (`McpServerSettings { enabled, port, token, categories, listenOnAllInterfaces }`) lives in `userData/ptnotes-settings.json` (chmod 600); missing/legacy `listenOnAllInterfaces` values normalize to `false`. IPC: `mcpServer:getStatus` / `mcpServer:update` / `mcpServer:regenerateToken`; the renderer pane shows status, port, token, per-category toggles, the network-access toggle and warning, detected endpoint selection, and a copy-ready client config snippet that uses the selected network URL in all-interface mode (or loopback otherwise). Changing categories or the listen mode restarts the host (existing sessions drop them).

## Web UI mode

One codebase, two front ends. The desktop app is untouched; a second, headless mode is started from its own process:

```bash
ptnotes web                                   # 127.0.0.1:9380
ptnotes web --host 0.0.0.0 --port 9380        # LAN
ptnotes web --doc-path ~/notes --data-path ~/.ptnotes-state
ptnotes web --token mycode                    # fixed access code
ptnotes web --no-auth                         # no access code (trusted networks only)
```

- **CLI** (`src/shared/cliArgs.ts`, pure): scans `process.argv` for the `web` subcommand (tolerating Electron's own flags and the injected app path, so `ptnotes web …`, `electron . web …` and `electron-vite dev -- web …` all work) and for `--flag value` / `--flag=value`. `--help` prints the usage and exits. `--data-path` sets `app.setPath('userData')` **before** any store is constructed (both modes); `--doc-path` creates the folder and persists it as `rootDir` (the same setting as Settings ▸ Storage). `~` is expanded for both.
- **Mode branch** (`src/main/index.ts`): `initCore()` holds everything both modes share — stores, services, bots/jobs/modules and every `rpc.handle` / `rpc.on` registration — and returns the service. Desktop additionally registers `ptfile://` + the menu/splash, calls `bindRegistryToIpc()` and installs the window broadcast sink. Web hides the dock, installs the SSE sink, starts the server, and quits on SIGINT/SIGTERM/SIGHUP (`window-all-closed` never quits it — there are no windows).
- **One API, two transports** (`src/shared/api.ts`): `createApi(transport)` is the single source of truth for `window.ptnotes` (the 566-line preload became this factory). The preload binds it to `ipcRenderer` (`src/preload/index.ts`); the renderer's `webBridge.ts` binds it to `fetch` + `EventSource` and is installed from `main.tsx` only when no preload ran. `ptnotes.ui.mode` is `'desktop' | 'web'` and is what the renderer branches on.
- **Registry** (`src/main/rpc/registry.ts`): one `RpcRegistry` singleton — `handle` (request/response), `on` (send-only), `channels()` / `pushChannels()` for the IPC adapter. Every call receives `InvokeCtx { send, platform }`; `ctx.send` goes to the calling window (desktop) or pushes an SSE frame (web). All 160 channel names are unchanged.
- **HTTP transport** (`src/main/rpc/httpServer.ts`, plain `node:http`, no new dependencies):
  - `POST /api/invoke` — `{ channel, args }` → `{ ok, result, error, actions }` with `rpcCodec` (Uint8Array ↔ `{__b64}`) on the way in and out; 128 MB body cap, so uploads ride through invoke as base64.
  - `GET /api/events` — SSE: every `broadcast(channel, payload)` and every `ctx.send`, plus heartbeats; auto-reconnects.
  - `GET /api/blob/<id>` — one-shot downloads (Excel export, diagram/image saves) held in memory for 10 minutes.
  - `GET /api/file?path=` — serves local files for images/PDF previews. **realpath-resolved and restricted to the project root** (`download=1` adds a `Content-Disposition` attachment).
  - `GET /` — the built renderer; in dev it proxies to `ELECTRON_RENDERER_URL` (including the WS upgrade) so same-origin holds.
- **Auth**: a random token (or `--token`) is printed as a deep link `http://127.0.0.1:9380/?t=<token>`, which exchanges it for an `HttpOnly; SameSite=Strict; Max-Age=2592000` `ptnotes_session` cookie. Unauthenticated page requests get an inline login form posting to `POST /api/session`; scripts may use `Authorization: Bearer`. Comparison is constant-time. Loopback binds validate `Host` against `127.0.0.1`/`localhost`/`::1` (DNS-rebinding), non-loopback binds print a warning, and an `Origin` header must match the request host.
- **Platform services** (`src/main/platform.ts`, `platformDesktop.ts`): dialogs/reveal/open behind `PlatformServices`. Web `showOpenDialog()` → `[]` and `showSaveDialog()` → `null`, so handlers branch on `ctx.platform.mode === 'web'` and answer with `putBlob(...)` + `emitClientAction(...)`. Client actions (`download` / `open`) are collected per request through an `AsyncLocalStorage` sink set by the HTTP adapter (the handler signatures are unchanged, and desktop never collects any) and executed by the web bridge: download via a synthetic `<a download>`, open via `window.open` with a download fallback.
- **Broadcast bus** (`src/main/rpc/bus.ts`) replaces the `BrowserWindow.getAllWindows()` loops. Desktop sink: every window (`jobs:event` `notify` still flashes the taskbar when unfocused). Web sink: every SSE client.
- **Uploads**: `files.getPathForFile` returns `''` in a browser, so the two new channels that carry bytes instead of a path exist: `files:copyBufferToProject(project, fileName, data)` (chat + editor drops, content-checked) and `files:importDroppedData(project, destSubpath, fileName, data)` (raw explorer drop). Gallery drops and "Add images" fall back to `gallery.importData`.
- **Renderer branches** (`src/renderer/src/uiMode.ts`, `isWebUi()`): reveal affordances relabel to **Download** with a download icon; Settings ▸ Storage edits the root path in a text field instead of opening a native picker; the gallery picker uses a hidden `<input type="file">`; `ptfile://` URLs become `/api/file?path=…` (`ptFileUrl` / `resolveImageSrc`); keyboard fallbacks for global find, menu Select All and the planner's undo/redo interception live in `webBridge.ts` (gated by the local `planner:set-edit-active` flag), and a capture-phase `contextmenu` listener cancels the browser's own menu so only the app's menus appear (Electron never showed one either).
- **Not run alongside the desktop UI**: don't point both at the same `--data-path` at the same time — `ptnotes-settings.json` is plain JSON with last-write-wins.
...
- **Web UI limitations (inherent, by design):** OS reveal → **Download**; `files:openExternal` → new tab (download fallback when a popup blocker intervenes); native folder/file pickers → a text field (root path), `<input type="file">` (gallery) and byte-upload drop handlers; right-click never opens the browser's context menu (only the app's own menus, or nothing); no splash screen, application menu, taskbar flash, window-state memory or Chromium PDF-plugin **Escape** (the overlay's ✕ still works); headful Browser-toolset windows open on the **server host**, not the browser's machine; chart/diagram/infographic/PDF workers stay in Electron utility processes on the server and render there. Without a browser connected to the session, only curl-level checks (server up, `/api/invoke`, `/api/events`, `/api/file`) are possible.
