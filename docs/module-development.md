...
- The subagent gets **all base chat tools** (`create_note`, `list_notes` (with `query` to search), `read_note`, `update_note`, `create_todos`, `toggle_todo`, `list_todos`, `read_file`, `web_search`, `web_fetch`, `read_skill` (with optional `file` param for sibling files)) **plus** the module's own tools (`module.tools`), **plus** the framework tools (`set_plan`, `update_step`).
