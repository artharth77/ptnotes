...
- The subagent gets **all base chat tools** (`create_note`, `list_notes` (with `query` to search), `read_note`, `update_note`, `list_kanban_cards`, `create_kanban_card`, `update_kanban_card`, `move_kanban_card`, `delete_kanban_card`, `read_file`, `web_search`, `web_fetch`, `read_skill` (with optional `file` param for sibling files)) **plus** the module's own tools (`module.tools`), **plus** the framework tools (`set_plan`, `update_step`).
- Modules can **read** skills: the runner injects the enabled-skills index (only when at least one skill is enabled) into the module's system prompt so it can discover and load skills via `read_skill` (with optional `file` param for sibling files). The mutating skill tools `create_skill` and `delete_skill` are **excluded** — modules never create/edit/delete skills.
