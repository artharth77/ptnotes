...
- The subagent gets **all base chat tools** (`create_note`, `list_notes`, `read_note`, `search_notes`, `update_note`, `create_todos`, `toggle_todo`, `list_todos`, `read_file`, `web_search`, `web_fetch`, `read_skill`, `read_skill_file`) **plus** the module's own tools (`module.tools`), **plus** the framework tools (`set_plan`, `update_step`).
