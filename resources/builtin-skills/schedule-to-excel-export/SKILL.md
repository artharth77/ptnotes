...
description: Exports project schedules and plans into professional, well-formatted Excel (.xlsx) workbooks based on defined design specifications.
enabled: true
---
...
- **Columns (Left to Right):**
  ...
  10. Status in % (Map to: %complete - _Note: Must be sent/formatted as a decimal value between 0 and 1, e.g., 18% is 0.18_)
...
- **Task Hierarchy Styling (applies to the ENTIRE ROW, i.e. all 10 cells from ITEM to Status in %):**
  ...
  - Note: The STATUS cell's conditional background (Light Yellow / Light Green / Light Red) still takes precedence for that **single STATUS cell only** over the row background.
...
| **OWNER**     | Center    | 17.50 |                                                                                                                                                                                                                             |
| **DUR**       | Center    | 5.33  |                                                                                                                                                                                                                             |
| **Dates & %** | Center    | 12.67 | Applies to START, END, Act. Start, Act. End, and Status in % (_% complete value must be between 0 and 1, e.g., 0.18 for 18%_)                                                                                               |
